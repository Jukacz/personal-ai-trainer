...
| `NEXT_PUBLIC_SITE_URL` | Site URL for sitemap and canonical URLs | `https://<your-vercel-domain>` |
...
- Root `docker-compose.yml` is for backend stack only (API + MongoDB); do not add frontend service there.
- In Vercel, configure:
  - `NEXT_PUBLIC_SITE_URL=https://<your-vercel-domain>`
  - `NEXT_PUBLIC_API_URL=https://<your-backend-domain>/api/v1`
- For local development, frontend still runs with `pnpm dev` on `http://localhost:3000`.
